...
window.sendAnalyticsEvent = function (eventType, extras) {
    sendCustomAnalyticsEvent(eventType, extras);
};
...
let isNormalGame =
    isBumperAd == true && isMRECEnabledOnLP != "true" ? false : true; //true means playing in browser
...
function showBumperAd() {
    console.log("showBumperAd");
    if (isMRECEnabledOnLP === "true") {
        console.log("isMRECEnabledOnLP");
        LPBannerInstance =
            window?.GlanceGamingAdInterface?.showLoadingPageBannerAd(
                LPMercObj,
                bannerCallbacks
            );
    } else {
...
function goToGame(setTimeoutSeconds = 0) {
    $("#gotoGame").click(() => {
        ...
        setTimeout(() => {
            ...
            console.log("refreshStickyBannerAdxxxx");
        }, setTimeoutSeconds);
    });
